...
from fsm2hdl.code_gen import generate_verilog
...
def test_generate_verilog_configurations(
    file_name: str,
    tmp_path: Path,
    fsm_type: FSMType,
    ...
    """
    Test whether all possible configurations for ``generate_verilog()`` yield output file.
    """
    ...
    generate_verilog(fsm, tmp_path.as_posix(), cfg)

    assert os.path.isfile(tmp_path.as_posix() + "/" + output_file_name + ".v")
